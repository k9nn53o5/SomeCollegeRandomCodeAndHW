...
SHPlayer::showCards() const
{
    ...
    for( int n = 0; n < nCards; n += kNCardPerRow ) {
        for( int height = 0; height < kCardHeight; height++ ) {
            cout << (height+1 > (int)strlen(name) ? ' ' : name[height]) << " ";
...
SHPlayer::isStraight() const
{
    ...
    if( sortedCards[0].getPip() == 0 &&
            sortedCards[1].getPip() == 9 &&
            sortedCards[2].getPip() == 10 &&
            sortedCards[3].getPip() == 11 &&
            sortedCards[4].getPip() == 12 )
        return true;
